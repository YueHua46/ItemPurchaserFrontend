...
const messageDropdownStore = useMessageDropdownStore()
onMounted(() => {
  // 初始化消息列表的dropdown操作实例
  messageDropdownStore.dropdownChange(
    new Dropdown(
      $targetEl.value,
      $triggerEl.value,
      messageDropdownStore.dropdownOptions
    )
  )
})
</script>
    ...
    <div class="divide-y divide-gray-100 dark:divide-gray-700">
      ...
      >
        <div class="flex-shrink-0">
          ...
          />
          <div
            class="absolute flex items-center justify-center w-5 h-5 ml-6 -mt-5 border border-white rounded-full dark:border-gray-800"
          >
            <svg
              class="w-3 h-3 text-white"
              aria-hidden="true"
